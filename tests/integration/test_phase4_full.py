...
class TestPhase4ApprovalFlow:
    ...
    def test_approval_workflow(self, test_client):
        ...
        query_mock = MagicMock()
        query_mock.filter.return_value.order_by.return_value.all.return_value = [mock_approval]
...
class TestPhase4WebSocket:
    ...
    def test_websocket_endpoint_exists(self, test_client):
        ...
        # 应该有WebSocket路由
        assert len(websocket_paths) > 0 or any("ws" in str(type(r)).lower() for r in app.routes)
