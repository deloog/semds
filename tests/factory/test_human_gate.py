...
class TestCheckApprovalQuality:
    ...
    def test_high_approval_rate_low_review_time_triggers_warning(self):
        ...
        monitor = HumanGateMonitor()
        # 99%批准率，平均3秒审查时间
        reviews = [Review(approved=True, duration_seconds=3) for _ in range(99)] + [
    ...
    def test_low_approval_rate_passes_check(self):
        ...
        monitor = HumanGateMonitor()
        # 50%批准率，平均3秒审查时间
        reviews = [Review(approved=True, duration_seconds=3) for _ in range(25)] + [
            Review(approved=False, duration_seconds=3) for _ in range(25)
        ]
...
class TestRecordReview:
    ...
    def test_record_review_adds_to_list(self):
        ...
        monitor.record_review(
            approved=True, duration_seconds=10.5, reviewer_id="user-1"
        )
...
class TestHumanGatePersistence:
    ...
    def test_record_review_limits_storage_size(self):
        ...
        for i in range(7):
            monitor.record_review(
                approved=True, duration_seconds=1.0, reviewer_id=f"reviewer-{i}"
            )
...
class TestAlertGeneration:
    ...
    def test_generate_alert_message(self):
        ...
        alert = monitor.generate_alert(approval_rate=0.99, avg_review_time=3.0)
