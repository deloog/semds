...
class EvolutionRunner:
    ...
    async def run(self):
        ...
        try:
            ...
            logger.info(f"Evolution completed: gens={result.generations}, score={result.best_score:.2f}")
    ...
    def _run_evolution_sync(self, requirements: list, test_code: str):
        ...
        try:
            ...
            if self._stop_requested:
                logger.info(f"Evolution stopped before starting for task {self.task_id}")
                return None
...
'''
    ...
    def _parse_requirements(self, task: Task) -> list:
        ...
        if task.description:
            # 按行分割，过滤空行
            lines = [line.strip() for line in task.description.split('\n') if line.strip()]
    ...
    def _save_final_result(self, db: Session, task: Task, result):
        ...
        db.commit()
        logger.info(f"Final result saved for task {self.task_id}: score={result.best_score:.2f}")
...
async def start_evolution_task(task_id: str, max_generations: int = 50) -> EvolutionRunner:
    """
    启动进化任务
