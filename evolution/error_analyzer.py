...
class ErrorAnalyzer:
    ...
    def _generate_summary(self, passed: int, failed: int, errors: int, failures: List[TestFailure]) -> str:
        """生成分析摘要"""
        total = passed + failed + errors
        
        lines = [
            f"Test Results: {passed}/{total} passed ({passed/total*100:.1f}%)" if total > 0 else "No tests run",
        ]
