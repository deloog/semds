...
class MetaLearner:
    ...
    def record_failure_and_fix(self, task_type: str, error_type: str,
        ...
        pattern_id = hashlib.md5(f"{task_type}:{error_signature}".encode()).hexdigest()[:12]
    ...
    def find_applicable_patterns(self, task_type: str, 
                                  error_type: str,
                                  error_message: str,
        ...
        for pattern in self.patterns.values():
            ...
            sig_similarity = self._signature_similarity(pattern.error_signature, current_sig)
    ...
    def record_strategy_result(self, strategy_name: str, task_type: str,
    ...
    def generate_enhanced_prompt(self, task_type: str, 
                                  base_prompt: str,
        ...
        recommended, rate = self.recommend_strategy(
            task_type, 
            ["minimal_constraints", "strong_constraints", "with_examples"]
        )
        
        if rate > 0.7:
            enhancements.append(f"【建议】对此类任务使用 '{recommended}' 策略（历史成功率 {rate:.0%}）")
    ...
    def _generate_signature(self, error_message: str, error_type: str) -> str:
        ...
        # 移除具体数值
        simplified = ' '.join(word for word in simplified.split() 
    ...
    def _find_similar_pattern(self, task_type: str, 
                              error_signature: str) -> Optional[FailurePattern]:
        """查找相似模式"""
        for pattern in self.patterns.values():
            if (pattern.task_type == task_type and 
                self._signature_similarity(pattern.error_signature, error_signature) > 0.7):
                return pattern
        return None
