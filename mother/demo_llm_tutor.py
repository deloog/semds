...
def demo_few_shot_optimizer():
    ...
    try:
        ...
        return {"error": str(e)}''',
            'improvements': ["Type hints", "Input validation", "Error handling", "Timeout"]
        },
        {
            'type': 'parser',
            'task': 'Parse CSV data',
    ...
    try:
        ...
        return []''',
            'improvements': ["Type hints", "Input validation", "Proper CSV parsing", "Error handling"]
        }
    ]
    ...
    prompt = optimizer.get_prompt_for_ollama(
        ...
        new_task="Download image from URL and save to file"
    )
