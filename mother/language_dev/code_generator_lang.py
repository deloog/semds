...
class LanguageAwareGenerator:
    ...
    def generate_code(
        ...
        # 生成代码
        print(f"[LangGenerator] Generating {language} code for: {task_description[:50]}...")
...
"""
    ...
    def generate_documentation(
        ...
        prompt = f"{prompts.get(doc_type, prompts['inline'])}\n\n```{language}\n{code}\n```"
    ...
    def validate_syntax(self, language: str, code: str) -> Dict:
        ...
        if not spec:
            return {"valid": True, "warnings": ["Language spec not found, skipping validation"]}
...
if __name__ == "__main__":
    ...
    for level, codes in examples.items():
        ...
        for example in codes:
            print(f"\n--- {example['task']} ---")
            print(example['code'][:500] + "..." if len(example['code']) > 500 else example['code'])
