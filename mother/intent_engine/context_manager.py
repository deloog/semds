...
class ContextManager:
    ...
    def get_or_create_session(self, session_id: str) -> ConversationContext:
        ...
        if session_id not in self.sessions:
            ...
            if len(self.sessions) >= self.max_sessions:
                oldest = min(self.sessions, 
                           key=lambda k: self.sessions[k].messages[-1].timestamp 
                           if self.sessions[k].messages else 0)
    ...
    def summarize_for_external_model(
        self, 
        session_id: str,
        current_task: str
    ) -> str:
    ...
    def get_session_stats(self, session_id: str) -> Dict:
        ...
        return {
            ...
            'models_used': list(set(
                msg.model_used for msg in session.messages 
                if msg.model_used
            )),
        }
