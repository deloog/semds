...
from mother.task_decomposer.decomposer import AtomicTask, TaskType, TaskStatus, TaskGraph
...
'''
        ...
        validation_passed = (
            quality_result['optimized_score'] >= 70 and
            line_count <= 50
        )
...
"""
    ...
    def execute_graph(self, graph: TaskGraph) -> bool:
        ...
        for task_id in graph.execution_order:
            ...
            print(f"[Executor] Progress: {completion:.0%} ({sum(1 for t in graph.tasks.values() if t.status == TaskStatus.COMPLETED)}/{len(graph.tasks)})")
    ...
    def generate_final_code(self, graph: TaskGraph) -> str:
        ...
        for task in graph.tasks.values():
            if task.task_type == TaskType.IMPLEMENT and task.status == TaskStatus.COMPLETED:
                implementations.append(task.actual_output)
            elif task.task_type == TaskType.TEST and task.status == TaskStatus.COMPLETED:
...
def execute_with_tdd(task_description: str) -> Tuple[bool, str]:
    ...
    if success:
        ...
        error_msg = "\n".join([
            f"Task {t.name} failed: {t.error_message}"
