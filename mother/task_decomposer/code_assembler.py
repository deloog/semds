...
class CodeAssembler:
    ...
    def assemble(self, atomic_tasks: List[AtomicTask]) -> List[CodeModule]:
        ...
        test_tasks = [t for t in atomic_tasks if t.task_type == TaskType.TEST]
        doc_tasks = [t for t in atomic_tasks if t.task_type == TaskType.ANALYSIS]
        ...
        for module in modules:
            if module.lines > self.TARGET_FILE_SIZE["max"]:
                ...
                if optimized and self._can_merge(optimized[-1], module):
                    print(
                        f"  [Optimize] Merging {module.name} into {optimized[-1].name}"
                    )
