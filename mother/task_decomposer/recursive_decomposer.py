...
class DecompositionValidator:
    ...
    def validate_decomposition(
        self, parent_task: str, subtasks: List[str]
    ) -> Tuple[bool, str]:
        """
        验证分解是否正确
    ...
    def _check_granularity(self, subtasks: List[str]) -> Dict:
        ...
        if len(subtasks) > 10:
            return {
                "appropriate": False,
                "reason": "Too many subtasks, consider grouping",
            }
        ...
        if len(subtasks) < 2:
            return {
                "appropriate": False,
                "reason": "Too few subtasks, needs more decomposition",
            }
...
class RecursiveDecomposer:
    ...
    def _create_atomic_task(self, description: str, force: bool = False) -> AtomicTask:
        """创建原子任务"""
