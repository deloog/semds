...
class CodeTutor:
    ...
    def _save_examples(self):
        ...
        with open(self.examples_db, "w", encoding="utf-8") as f:
            json.dump(
                [asdict(e) for e in self.examples], f, indent=2, ensure_ascii=False
            )

    def teach(
        self, task: str, student_code: str, max_iterations: int = 3
    ) -> TutorSession:
        """
        教学生改进代码
        """
        ...
        for i in range(max_iterations):
            ...
            # 4. 生成改进后的代码（用教师模型）
            improved = self._generate_improved_code(task, current_code, feedback)
    ...
    def _generate_feedback(self, task: str, code: str, assessment: Dict) -> Dict:
        """生成改进反馈"""
    ...
    def _generate_improved_code(
        self, task: str, current_code: str, feedback: Dict
    ) -> Dict:
        """用教师模型生成改进后的代码"""
        task_spec = {
    ...
    def _record_learning_example(
        self, task: str, bad_code: str, good_code: str, iterations: List[Dict]
    ):
        """记录学习示例"""
        improvements = []
...
"""
    ...
    session = tutor.teach(task="Fetch data from URL safely", student_code=student_code)
