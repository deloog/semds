...
class ComparisonExperiment:
    ...
    def run_experiment(self, task: str) -> dict:
        ...
        return {"task": task, "method1": result1, "method2": result2}
    ...
    def _method_decomposition(self, task: str) -> dict:
        ...
        try:
            ...
            atomic_tasks = [
                t for t in graph.tasks.values() if t.task_type.value in ["implement"]
            ]
    ...
    def _method_direct(self, task: str) -> dict:
        ...
        try:
            ...
            result = self.local_model.generate(
                {"description": task, "requirements": ["Complete code", "Type hints"]}
            )
    ...
    def print_comparison(self, result: dict):
        ...
        # Success
        s1, s2 = ("[OK]" if m1["success"] else "[FAIL]"), (
            "[OK]" if m2["success"] else "[FAIL]"
        )
        print(f"{'Success':<25} {s1:<20} {s2:<20}")

        # Quality
        print(
            f"{'Quality Score':<25} {m1['quality']:.1f}/100{'':<12} {m2['quality']:.1f}/100"
        )
