...
class SystemTelemetry:
    ...
    def analyze_recent_performance(self, hours: int = 24) -> Dict:
        ...
        # 计算关键指标
        code_gen_obs = [
            o for o in observations if o["metric_name"] == "code_generation"
        ]
        ...
        avg_score = sum(o["context"].get("score", 0) for o in code_gen_obs) / len(
            code_gen_obs
        )
    ...
    def _identify_improvements(
        self, error_counts: Dict, success_rate: float
    ) -> List[str]:
        """基于错误模式识别改进机会"""
        opportunities = []
...
class SelfExperiment:
    ...
    def run_ab_test(
        self, hypothesis: ImprovementHypothesis, n_samples: int = 10
    ) -> ExperimentResult:
        """
        运行A/B测试验证假设
        ...
        for i in range(n_samples):
            ...
            treatment_code = self._generate_with_config(
                test_case[0], hypothesis.proposed_change
            )
...
class SafeSelfUpdater:
    ...
    def apply_improvement(
        self, hypothesis: ImprovementHypothesis, experiment_result: ExperimentResult
    ) -> bool:
        """
        安全地应用验证通过的改进
...
class MetaEvolutionEngine:
    ...
    def run_evolution_cycle(self) -> Dict:
        ...
        for hypothesis in hypotheses:
            ...
            print(
                f"    Significant: {'YES' if experiment_result.is_significant else 'NO'}"
            )

            results["details"].append(
                {"hypothesis": hypothesis, "experiment": experiment_result}
            )
...
def demo_meta_evolution():
    ...
    for i in range(3):
        engine.record_generation_result(
            task_type="calculator", success=True, score=0.8, generation_time=3.0
        )
