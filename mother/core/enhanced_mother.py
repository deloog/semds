...
class EnhancedMotherSystem(MotherSystem):
    ...
    def _execute_plan(self, plan: ExecutionPlan) -> Dict:
        ...
        for i, step in enumerate(plan.steps, 1):
            ...
            try:
                ...
                # 尝试使用搜索解决问题
                solution = self._attempt_error_recovery(
                    error_msg, step.action, inputs
                )
    ...
    def _attempt_error_recovery(
        ...
        solutions = self.searcher.search_for_solution(
            error, 
            context=f"{action} with inputs {list(inputs.keys())}"
        )
        
        if solutions and solutions[0]['confidence'] > 0.5:
            best = solutions[0]
            print(f"[Recovery] Found potential solution (confidence: {best['confidence']:.2f})")
            print(f"  Source: {best['source']}")
            print(f"  Suggestion: {best['suggestion'][:100]}...")
            
            self.solutions_applied.append({
                'type': 'search_solution',
                'error': error,
                'solution': best
            })
    ...
    def print_self_report(self):
        ...
        if 'search_queries' in kb:
            ...
            successful = sum(
                1 for q in kb['search_queries'].values() 
                if q.get('successes', 0) > 0
            )
    ...
    for task in tasks:
        ...
        if result['success']:
            ...
            if 'extracted_data' in result.get('outputs', {}):
                ...
                print(f"  Found: {len(data) if isinstance(data, list) else 'N/A'} items")
        else:
