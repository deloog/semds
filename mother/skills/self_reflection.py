...
class SelfReflection:
        ...
        if os.path.exists(history_file):
            try:
                with open(history_file, "r", encoding="utf-8") as f:
                    data = json.load(f)
                    self.execution_history = [TaskExecution(**item) for item in data]
    ...
    def _extract_lessons(
        self, success: bool, errors: List[str], solutions: List[Dict]
    ) -> List[str]:
        """从执行中提取经验教训"""
        lessons = []
    ...
    def generate_reflection_report(self) -> Dict:
        ...
        if not self.execution_history:
            return {"total_tasks": 0, "message": "No execution history yet"}
        ...
        # 生成建议
        suggestions = self._generate_suggestions(successful, failed, common_errors)
    ...
    def _generate_suggestions(
        self, successful: int, failed: int, common_errors: List
    ) -> List[str]:
        """生成改进建议"""
        suggestions = []
    ...
    def learn_from_search(self, query: str, results: List[Dict], success: bool):
        """
        从搜索结果学习
