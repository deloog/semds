...
class SecurityChecker:
    ...
    def _check_hardcoded_secrets(self, code: str) -> List[Optimization]:
        ...
        issues = []
        patterns = [
            (r'["\']\s*(?:api[_-]?key|password|secret|token)\s*["\']\s*=\s*["\'][^"\']+["\']', 'HARDCODED_SECRET'),
...
class CodeOptimizer:
    ...
    def _generate_suggestions(self, issues: List[Optimization]) -> List[str]:
        ...
        if 'CODE_TOO_LONG' in error_types:
            suggestions.append("Consider splitting the tool into smaller functions or separate tools")
        ...
        if not suggestions:
            suggestions.append("Code looks good! Consider adding more docstrings for clarity.")
