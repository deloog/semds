...
class FullPipelineExperiment:
    ...
    def __init__(self):
        self.meta_learner = MetaLearner(
            storage_path="experiments/integration_meta_db.json"
        )
...
"""
    ...
    def _run_scene_2(self) -> Dict:
        ...
        task = TaskSpec(
            ...
            constraints=["函数名必须是 'eval_expr'", "正确处理运算符优先级"],
            examples=[Example("3+4*2", 11.0, "优先级测试")],
        )
        ...
        enhanced_prompt = self.meta_learner.generate_enhanced_prompt(
            task_type="calculator", base_prompt=base_prompt, error_history=error_history
        )
        ...
        print(
            "  [Simulation] With historical guidance, code includes precedence handling"
        )
    ...
    def _analyze_failures(self, result: Dict):
        ...
        return self.error_analyzer.analyze(test_output, total_tests=passed + failed)
...
'''
    ...
    def _print_comparison(self, scene1: Dict, scene2: Dict):
        ...
        checks = [
            ...
            (
                "Code fix applied",
                scene1.get("final_score", 0) > scene1.get("initial_score", 0),
            ),
