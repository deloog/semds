...
class SelfValidator:
    ...
    def validate(
        self, code: str, test_runner: TestRunner = None, test_code: str = None
    ) -> tuple[bool, str, dict]:
        """
        验证代码并给出修正建议
...
"""
    ...
    def run_generation_with_validation(
        self, generator: CodeGenerator, gen_num: int, previous_feedback: str = None
    ) -> dict:
        """运行一代进化，包含自验证和重试机制"""
        ...
        for retry in range(self.max_retries):
            ...
            result = generator.generate(
                task_spec=task_spec, previous_code=None  # Phase 1 不使用前代代码
            )
            ...
            if is_valid:
                ...
                # 运行完整测试获取详细结果
                test_result = self.validator._run_test(
                    code, self.test_runner, self.test_code
                )
                return {
                    "success": True,
    ...
    def _print_report(
        self, history: list, best_score: float, best_code: str, elapsed: float
    ):
        """打印实验报告"""
        ...
        goals = {
            ...
            "Auto-retry mechanism working": len(history) > 0
            and any(h.get("retries", 0) > 0 for h in history),
