...
TEST_FILE_PATH = PROJECT_ROOT / "experiments" / "calculator" / "tests" / "test_string_calculator.py"
...
def run_evolution():
    ...
    try:
        ...
        for gen_result in result.history:
            strategy = gen_result.strategy.get('mutation_type', 'unknown') if gen_result.strategy else 'unknown'
            passed = 'YES' if gen_result.passed_tests else 'NO'
            print(f"{gen_result.generation:<5} {gen_result.score:<10.4f} {passed:<10} {strategy:<20}")
