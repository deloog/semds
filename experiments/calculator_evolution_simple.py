...
def create_strategy(generation: int, previous_score: float, failed_tests: list) -> dict:
    """创建进化策略"""
    if generation == 0:
        return {"mutation_type": "conservative", "improvement_focus": "实现基本四则运算", "temperature": 0.5}
    
    if previous_score < 0.5:
        return {"mutation_type": "aggressive", "improvement_focus": f"重写核心逻辑，修复: {', '.join(failed_tests[:3])}", "temperature": 0.7}
    
    if previous_score < 0.8:
        return {"mutation_type": "hybrid", "improvement_focus": f"修复失败测试: {', '.join(failed_tests)}", "temperature": 0.5}
    
    if previous_score < 0.95:
        return {"mutation_type": "conservative", "improvement_focus": f"优化边界: {', '.join(failed_tests[:2])}", "temperature": 0.3}
    
    return {"mutation_type": "conservative", "improvement_focus": "代码清理", "temperature": 0.2}
...
def generate_report(results, best_score, best_gen, best_code, start_time):
    ...
    print(f"\n📈 进化轨迹:")
    print("-"*70)
    print(f"{'Gen':<5} {'Pass Rate':<12} {'Passed':<8} {'Strategy':<15} {'Improvement'}")
    print("-"*70)
    
    for r in results:
        improvement = r['improvement']
        imp_str = f"+{improvement:.2%}" if improvement > 0 else f"{improvement:.2%}" if improvement < 0 else "-"
        print(f"{r['gen']:<5} {r['pass_rate']:<12.2%} {r['passed']:<8} {r['strategy']:<15} {imp_str}")
    ...
    with open(report_file, 'w', encoding='utf-8') as f:
        ...
        f.write("-"*70 + "\n")
        for r in results:
            f.write(f"Gen {r['gen']}: {r['pass_rate']:.2%} ({r['passed']}/10) - {r['strategy']}\n")
        f.write("-"*70 + "\n\n")
