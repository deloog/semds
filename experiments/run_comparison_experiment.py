...
def run_single_experiment(
    ...
    if use_consilium:
        ...
        result_data["results"]["consilium_deliberation"] = {
            ...
            "safety_level": deliberation_result.get("guardian_review", {}).get("safety_level"),
            "time_seconds": round(deliberation_time, 2)
        }
        
        print(f"  审议结果: {deliberation_result.get('recommendation')}")
        print(f"  安全级别: {deliberation_result.get('guardian_review', {}).get('safety_level')}")
    ...
    if deliberation_result and consilium.ConsiliumEngine().is_safe_to_proceed(deliberation_result):
        # 根据审议结果可能调整策略
        pass
    ...
    if use_consilium:
        ...
        result_data["results"]["consilium_review"] = {
            "safety_level": review_result.get("guardian_review", {}).get("safety_level"),
            "risk_level": engine.get_risk_level(review_result),
            "time_seconds": round(review_time, 2)
        }
        
        print(f"  审查安全级别: {review_result.get('guardian_review', {}).get('safety_level')}")
...
def generate_summary():
    ...
    for r in all_results:
        ...
        summary[group]["pass_rates"].append(r["results"].get("testing", {}).get("pass_rate", 0))
        summary[group]["avg_gen_time"].append(r["results"].get("generation", {}).get("time_seconds", 0))
    ...
    for group, data in sorted(summary.items()):
        avg_pass = sum(data["pass_rates"]) / len(data["pass_rates"]) if data["pass_rates"] else 0
        avg_time = sum(data["avg_gen_time"]) / len(data["avg_gen_time"]) if data["avg_gen_time"] else 0
    ...
    # 保存完整报告
    report_path = RESULTS_DIR / f"experiment_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
