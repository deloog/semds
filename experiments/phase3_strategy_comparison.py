...
class StrategyAwareCodeGenerator:
    ...
    def generate(self, task_spec, strategy=None, **kwargs):
        ...
        # 从策略中提取参数
        mutation_type = strategy.get("mutation_type", "hybrid") if strategy else "hybrid"
...
def run_strategy(strategy_name: str, mutation_type: str, temperature: float, 
                 num_runs: int = 5) -> List[StrategyResult]:
    """
    运行单一策略多次，收集统计信息。
...
def analyze_strategy_results(results: List[StrategyResult]) -> Dict:
    ...
    # 收敛稳定性（得分方差）
    score_variance = sum((r.final_score - avg_final_score) ** 2 for r in results) / len(results)
...
def run_strategy_comparison():
    ...
    for name, s in stats.items():
        stability = "高" if s['score_variance'] < 0.01 else "中" if s['score_variance'] < 0.03 else "低"
        print(f"{name:<15} {s['success_rate']*100:>6.1f}%    "
    ...
    for name, results in all_results.items():
        s = stats[name]
        print(f"\n{name} 策略:")
        print(f"  成功率: {s['success_rate']*100:.1f}% ({s['success_rate']*5:.0f}/5 次成功)")
        ...
        for i, r in enumerate(results, 1):
            ...
            print(f"    [{status}] Run {i}: {r.generations}代, 得分{r.final_score:.3f}, {r.duration:.2f}s")
