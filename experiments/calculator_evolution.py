...
def create_strategy(generation: int, previous_score: float, failed_tests: list) -> dict:
    ...
    if previous_score < 0.5:
        ...
        return {
            ...
            "improvement_focus": "完全重写核心逻辑，参考失败测试: "
            + ", ".join(failed_tests[:3]),
            "temperature": 0.7,
        }
    ...
    if previous_score < 0.95:
        ...
        return {
            "mutation_type": "conservative",
            "improvement_focus": "优化边界情况处理，提高健壮性，关注: "
            + ", ".join(failed_tests[:2]),
            "temperature": 0.3,
        }
        ...
        for gen in range(TERMINATION_CONFIG["max_generations"]):
            ...
            if gen > 0:
                prev_gen = (
                    ...
                    .filter_by(task_id=task.id, gen_number=gen - 1)
                    .first()
                )
                if prev_gen:
...
def print_summary(session, task):
    ...
    for gen in generations:
        strategy = (
            gen.strategy_used.get("mutation_type", "unknown")
            if gen.strategy_used
            else "unknown"
        )
        ...
        print(
            f"{gen.gen_number:<5} {gen.test_pass_rate:<12.2%} {strategy:<15} {improvement_str}"
        )
