    ...
    with tempfile.TemporaryDirectory(prefix="semds_test_") as work_dir:
        ...
        try:
            ...
            tests = [
                ...
                ("test_division_by_zero", lambda: (_raises_value_error(lambda: calculate(1, 0, '/')))),
                ("test_invalid_operator", lambda: (_raises_value_error(lambda: calculate(1, 2, '%')))),
                ("test_negative_numbers", lambda: calculate(-3, -2, '*') == 6),
                ("test_float_numbers", lambda: abs(calculate(0.1, 0.2, '+') - 0.3) < 1e-9),
