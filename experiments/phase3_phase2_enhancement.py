    ...
    def generate_fuzz_cases(self, base_cases: List[Dict]) -> List[Dict]:
        ...
        for case in base_cases:
            ...
            # 列表变异
                ...
                if len(input_val) > 0:
                    mutations.extend([
                        {"input": input_val + [999], "type": "append", "expected": None},
                        {"input": input_val[:-1], "type": "truncate", "expected": None},
                        {"input": input_val[::-1], "type": "reverse", "expected": None},
                    ])
            ...
            # 字符串变异
                ...
                mutations.extend([
                    {"input": input_val + "x", "type": "append_char", "expected": None},
                    {"input": input_val[:-1] if len(input_val) > 0 else "", "type": "truncate", "expected": None},
                    {"input": input_val.upper(), "type": "uppercase", "expected": None},
                ])
        
        return mutations[:10]  # 限制变异数量
    
    def test_robustness(self, code: str, func_name: str, base_cases: List[Dict]) -> float:
        """测试代码对输入变化的鲁棒性"""
...
class BehaviorConsistencyChecker:
    ...
    def check_properties(self, code: str, func_name: str) -> Dict[str, bool]:
        ...
        try:
            ...
            # 字符串反转检查
            elif "reverse" in func_lower or ("string" in func_lower and "reverse" in code.lower()):
...
class Phase2EnhancedEvaluator:
    ...
    def evaluate(self, code: str, function_signature: str, 
                 base_cases: List[Dict] = None) -> Dict[str, Any]:
        """
        执行完整评估。
        ...
        # 1. 基础评估
        base_result = self.base_evaluator.evaluate(
        ...
        # 3. 复杂度检测
        complexity_label, complexity_score = self.complexity_checker.evaluate_complexity(
            code, func_name
        )
        
        # 4. 行为一致性
        properties = self.behavior_checker.check_properties(code, func_name)
        consistency_score = sum(properties.values()) / len(properties) if properties else 0.5
...
def run_phase2_experiment():
    ...
    for test in test_cases:
        ...
        print(f"{test['name']:<25} {base_score:<12.2f} {phase2_score:<12.2f} {complexity:<15}")
    ...
    for r in results:
        ...
        print(f"  提升: {((r['phase2_score'] - r['base_score']) / r['base_score'] * 100):+.0f}%")
        ...
        if d['consistency']['properties']:
            print(f"    - 满足的性质: {', '.join(k for k, v in d['consistency']['properties'].items() if v)}")
    ...
    avg_improvement = sum(
        (r['phase2_score'] - r['base_score']) / r['base_score'] 
        for r in results
    ) / len(results) * 100
