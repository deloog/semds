...
""",
    ...
    def run_task(
        self, task_spec: TaskSpec, with_constraints: bool = True
    ) -> TaskResult:
        """运行单个任务"""
    ...
    def run_experiment(self):
        ...
        for task in tasks:
            ...
            for attempt in range(3):
                ...
                print(
                    f"  Attempt {attempt+1}: {status} (func_name: {result.function_name_correct}, score: {result.score:.0%})"
                )
    ...
    def _print_report(
        self, results_with: List[TaskResult], results_without: List[TaskResult]
    ):
        """打印实验报告"""
        ...
        print("-" * 70)
        print(
            f"{'Total attempts':<30} {stats_with['total']:<20} {stats_without['total']:<20}"
        )
        print(
            f"{'Success rate':<30} {stats_with['success']/stats_with['total']:.1%}              {stats_without['success']/stats_without['total']:.1%}"
        )
        print(
            f"{'Function name correct':<30} {stats_with['func_correct']/stats_with['total']:.1%}              {stats_without['func_correct']/stats_without['total']:.1%}"
        )
        print(
            f"{'Average score':<30} {stats_with['avg_score']:.1%}              {stats_without['avg_score']:.1%}"
        )
        print("-" * 70)
